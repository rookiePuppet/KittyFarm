...
import { PromiseWithTimeout } from "@bundle:com.PuppetStudio.KittyFarm/entry/ets/utils/PromiseWithTimeout";
export class TuanjieLocation {
    static requestLocationUpdates(timeInterval: number, distanceInterval: number, accuracy: number) {
        ...
        let locationChange = (location: geoLocationManager.Location): void => {
            TuanjieLog.info('Update Location. Data: %{public}s', JSON.stringify(location) ?? '');
        ...
        try {
            ...
            TuanjieLog.error('Update Location Failed. Error: %{public}s', JSON.stringify(err) ?? '');
        }
    }
    static removeUpdates() {
        try {
            ...
            TuanjieLog.error('Remove Updates Failed. Error: %{public}s', JSON.stringify(err) ?? '');
        }
    }
