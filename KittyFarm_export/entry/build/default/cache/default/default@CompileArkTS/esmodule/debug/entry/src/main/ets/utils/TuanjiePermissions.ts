...
import { TuanjieMainWorker } from "@bundle:com.PuppetStudio.KittyFarm/entry/ets/workers/TuanjieMainWorker";
...
export class TuanjiePermissions {
    static requestUserPermissions(permissionsStr: Array<string>, onGranted: number, onDenied: number): void {
        ...
        atManager.requestPermissionsFromUser(context, permissions, (err, data) => {
            if (err) {
                TuanjieLog.error('Failed to requestPermission. Cause: %{public}s', JSON.stringify(err) ?? '');
            }
            else {
                TuanjieLog.info('Succeeded in requestPermission. Data: %{public}s', JSON.stringify(data) ?? '');
    ...
    static async checkAccessToken(permission: Permissions): Promise<abilityAccessCtrl.GrantStatus> {
        ...
        try {
            ...
            TuanjieLog.error('Failed to getBundleInfoForSelf. Cause: %{public}s', JSON.stringify(err) ?? '');
        }
        ...
        try {
            ...
            TuanjieLog.error('Failed to checkAccessToken. Cause: %{public}s', JSON.stringify(err) ?? '');
        }
        return grantStatus;
    }
    ...
    static async checkPermission(permissionsStr: string, onAuthorized: number, onUnauthorized: number): Promise<void> {
        ...
        try {
            ...
            TuanjieLog.error('Failed to checkPermission. Cause: %{public}s', JSON.stringify(err) ?? '');
        }
        finally {
            TuanjieMainWorker.getInstance().postMessage({
